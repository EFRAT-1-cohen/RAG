...
השתמשתי בארכיטקטורת Event-Driven שבה ה-LLM משמש כנתב (Router).
...
🛠️ טכנולוגיות בשימוש
LlamaIndex Workflows: לניהול זרימת האירועים.

Pinecone & Cohere: לאחסון וקטורי וביצוע חיפוש סמנטי (MVP).

Structured Extraction: חילוץ מידע לסכמת JSON מובנית (החלטות, כללים ואזהרות).
...
"האם יש רכיבים רגישים שצריך להיזהר בהם?" (שליפה מובנית)"# RAG" 
